...
<body>
    <header>
        <nav>
            ...
            <h2 class="nav__title">Tabel Pegawai</h2>
        </nav>
    </header>
...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $conn->query("DELETE FROM Pegawai WHERE id_Pegawai='$id'");
}
...
if (isset($_POST['update'])) {
    ...
    $sql_update = "UPDATE Pegawai SET 
        Nama='$nama', Jabatan='$jabatan', No_Telepon='$no_telepon', Email='$email',
        Tanggal_Masuk='$tanggal_masuk', Gaji='$gaji'
        WHERE id_Pegawai='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['update'])) {
    ...
    $sql_insert = "INSERT INTO Pegawai (id_Pegawai, Nama, Jabatan, No_Telepon, Email, Tanggal_Masuk, Gaji) 
                   VALUES ('$id_pegawai', '$nama', '$jabatan', '$no_telepon', '$email', '$tanggal_masuk', '$gaji')";

    $conn->query($sql_insert);
}

// Fetch data from the 'Pegawai' table
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>
                    <a href='?delete_id={$row['id_Pegawai']}'>Hapus</a> 
...
<form method="post" action="">
    <label>ID Pegawai:</label><input type="number" name="id_Pegawai" required><br><br>
    <label>Nama:</label><input type="text" name="Nama" required><br><br>
    <label>Jabatan:</label><input type="text" name="Jabatan" required><br><br>
    <label>No Telepon:</label><input type="number" name="No_Telepon" required><br><br>
    <label>Email:</label><input type="email" name="Email" required><br><br>
    <label>Tanggal Masuk:</label><input type="date" name="Tanggal_Masuk" required><br><br>
    <label>Gaji:</label><input type="number" step="0.01" name="Gaji" required><br><br>
