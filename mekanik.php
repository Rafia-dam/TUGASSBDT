...
<nav>
    ...
    <h2 class="nav__title">Tabel Mekanik</h2>
</nav>
</header>
...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $conn->query("DELETE FROM Mekanik WHERE id_Mekanik='$id'");
}
...
if (isset($_POST['update'])) {
    ...
    $sql_update = "UPDATE Mekanik SET 
        Nama='$nama', Keahlian='$keahlian', No_Telepon='$no_telepon', Email='$email'
        WHERE id_Mekanik='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['update'])) {
    ...
    $sql_insert = "INSERT INTO Mekanik (id_Mekanik, Nama, Keahlian, No_Telepon, Email) 
                   VALUES ('$id_mekanik', '$nama', '$keahlian', '$no_telepon', '$email')";

    $conn->query($sql_insert);
}

// Fetch data from the 'Mekanik' table
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>{$row['Email']}</td>
                <td>
                    <a href='?delete_id={$row['id_Mekanik']}'>Hapus</a> 
                    <a href='edit_mekanik.php?id={$row['id_Mekanik']}'>Edit</a>
                </td>
              </tr>";
    }
    echo "</table>";
} else {
    echo "Tidak ada data mekanik.";
}
...
<form method="post" action="">
    <label>ID Mekanik:</label><input type="number" name="id_Mekanik" required><br><br>
    <label>Nama:</label><input type="text" name="Nama" required><br><br>
    <label>Keahlian:</label><input type="text" name="Keahlian" required><br><br>
    <label>No Telepon:</label><input type="text" name="No_Telepon" required><br><br>
